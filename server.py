...
class Router():
    ...
    def forward_packet(self, packet, destination_addr):
        if isinstance(packet, Packet):
            if self.knows_route(destination_addr):
            ...
            print(f"Sending location request packet to all connected nodes")

            location_request_packet = LocationRequestPacket(
                self.byte_addr, packet.destination_address
            )
                ...
                try:
                    ...
                    # Check if the packet is not from this router itself
                        if response_packet.source_address != self.byte_addr:
                            print(f"Response received from {response_packet.source_address}")
                            ...
                            if self.hasPacket == 1:
                                self.forward_packet(packet, destination_addr)
                            break
                    else:
    ...
    def listen(self):
        while True:
            ...
            if isinstance(received_packet, LocationRequestPacket):
                print(f"Received location request packet from {received_packet.source_address} for {received_packet.destination_address}")
                if self.knows_route(received_packet.destination_address):
                    ...
                    print(f"Sending location response packet to {received_packet.source_address}")
                    self.socket.sendto(response_packet.to_bytes(), addr)
                else:
                    self.forward_packet(received_packet, received_packet.destination_address)
                    if self.knows_route(received_packet.destination_address):
                        response_packet = LocationResponsePacket(
                            self.byte_addr,
                            addr,
                ...
                self.hasPacket = 1
                print(f"Received packet at router {self.byte_addr}: {received_packet.data}")
                self.forward_packet(received_packet, received_packet.destination_address)
    ...
    def update_routing_table(self, addr, node, byte_addr):
        ...
        ip, port = node
        self.routing_table.add_route(addr, ip, port, 300, byte_addr)
